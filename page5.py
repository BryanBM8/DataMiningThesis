...
def show():
    ...
    tooltip=['day:N', 'Total Tweet:Q']
        ...
        width=400,
        height=300,
        title="Sarcastic Tweet Distribution per Day"
    ).configure_axisX(
        labelAngle=-45
    )
    ...
    grouped = sarcastic_df.groupby(['hour']).size().reset_index(name='Total Tweet')
    ...
    chart = alt.Chart(grouped).mark_bar().encode(
        ...
        width=400,
        height=300,
        title="Sarcastic Tweet Distribution per Hour"
    )
